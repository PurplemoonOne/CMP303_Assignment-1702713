...
public:
	...
	{
		sf::Vector2f direction = entity->GetTransform(entity->GetTag()).position - other->GetTransform(other->GetTag()).position;
		other->GetTransform(other->GetTag()).position += direction * 60.0f * deltaTime;
	}
